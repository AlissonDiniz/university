...
class Nota {
    ...
    public function get($matriculaTurmaDisciplina, $numeroEtapa) {
        $conection = new Conection();
        $query = "SELECT id FROM nota WHERE matricula_turma_disciplina_id = '$matriculaTurmaDisciplina' AND numero_etapa = '$numeroEtapa'";
        $result = $conection->selectQuery($query);
        $array = $conection->fetch($result);
        return $array[0];
    }

    public function update($matriculaTurmaDisciplina, $numeroEtapa, $valor, $usuario) {
        $conection = new Conection();
        $idNota = $this->get($matriculaTurmaDisciplina, $numeroEtapa);
