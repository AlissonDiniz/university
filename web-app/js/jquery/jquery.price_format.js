...
(function($) {
    ...
    $.fn.notaFormat = function(options) {
        ...
        return this.each(function() {
            ...
            function price_format () {
                ...
                // format to fill with zeros when < 100
                while (formatted.length<3) formatted = '0'+formatted;
                var centsVal = formatted.substr(formatted.length-1,1);
                var integerVal = formatted.substr(0,formatted.length-1);
                ...
                if (thousandsSeparator) {
                    ...
                    for (var j=integerVal.length;j>0;j--) {
                        char = integerVal.substr(j-1,1);
                        thousandsCount++;
                        if (thousandsCount%2==0) char = thousandsSeparator+char;
                        thousandsFormatted = char+thousandsFormatted;
                    }
                    if (thousandsFormatted.substr(0,1)==thousandsSeparator) thousandsFormatted = thousandsFormatted.substring(1,thousandsFormatted.length);
                    formatted = thousandsFormatted+centsSeparator+centsVal;
